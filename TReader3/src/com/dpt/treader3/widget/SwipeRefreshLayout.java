...
public class SwipeRefreshLayout extends ViewGroup {
	...
	private static final float MAX_SWIPE_DISTANCE_FACTOR = 1.0f;// 阻力
	private static final int REFRESH_TRIGGER_DISTANCE = 120;// 触发距离

	// treader
	private static final boolean mIsElasticity = false;// 是否有弹力效果
	...
	private final AnimationListener mShrinkAnimationListener = new BaseAnimationListener() {
		...
		public void onAnimationEnd(Animation animation) {
			...
			if (mTriggerListener != null) {
				mTriggerListener.onTrigger(mCurrPercentage);
			}
		}
	};
			...
			if (mDownEvent != null && !mReturningToStart) {
				...
				if (yDiff > mTouchSlop) {// 已经下拉刷新
					...
					if (yDiff > mDistanceToTriggerSync) {
						...
						// 只是跟踪用户的运动
						float interpolation = mAccelerateInterpolator
								.getInterpolation(yDiff
										/ mDistanceToTriggerSync);
						// LogHelper.e("SwipeRefreshLayout interpolation",interpolation+"");
						setTriggerPercentage(interpolation);
						// LogHelper.e("SwipeRefreshLayout yDiff / mDistanceToTriggerSync",
						// (yDiff / mDistanceToTriggerSync)+"");
						float offsetTop = yDiff;
						if (mPrevY > eventY) {
							offsetTop = yDiff - mTouchSlop;
						}
						// LogHelper.e("SwipeRefreshLayout offsetTop",
						// (offsetTop)+"");
						// LogHelper.e("SwipeRefreshLayout mTouchSlop",
						// (mTouchSlop)+"");
						updateContentOffsetTop((int) (offsetTop));
						if (mIsElasticity) {
							if (mPrevY > eventY
									&& (mTarget.getTop() < mTouchSlop)) {
